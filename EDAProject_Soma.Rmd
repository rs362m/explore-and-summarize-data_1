...
In the above plot I have limited the y-axis scale to 20 to get a better picture of the Box Plot. Ptot depicts that Delinquencies are more for 36 months Term Loans than other Terms. Correlation Value is -0.0838 and the value without zero's and 'NA's is -0.0996. Below is the number of deliquencies for each Term.
...
# Sum of Deliquencies per year
by(dfTemp$CurrentDelinquencies,dfTemp$CreditScoreRangeUpper,summary)


```

In the above plot, the  median of delinquencies is high at credit score of 439, 459 and dereased by increase in credit score upto 659 and stays a 1 after that. Borrowers with credit score of 659 has highest sum of deliquencies. The number of deliquencies decreased from 759.
...
ggplot(LoanData, mapping = aes(x = BorrowerRate,
  ...
  layer(geom = "bar", 
        stat = "bin",
        position="stack",
        stat_params = list(binwidth = 0.005)) +
  scale_fill_brewer(palette="OrRd") +
  scale_x_continuous(breaks = seq(0.3,0.37, 0.01),
                     limits = c(0.3,0.37)) +
  scale_y_continuous(breaks = seq(0,4000, 500),
...
From the plot we can that in 2006 all of the deliquencies were from the borrowers with income range "Not displayed". Also there is a  drop of number of deliquencies in 2009. Now let's see the same based on Home owner type.
...
Here in the above plot we can see that Non Home owner borrowers deliquencies are bit more than Home owner borrowers deliquencies in the years 2006 - 2008.
...
In Bi-variate section I saw that the  median of delinquencies is high at credit score of 439, 459 and dereased by increase in credit score upto 659 and remains a 1 after that. In Multi-Variate section I saw some Non Homeowner borrowers with high deliquencies have credit score of appx 500 and I think their Income Range is 'Not displayed'. In case of Non Homeowners all the borrowers with Income Range of 'Not displayed' are clustered between credit scores of 450-550. Now I want to see how the number of deliquencies varies with credit score based on borrowers employment status.

In the above plot I am considering Deliquencies which are greater than 1 (as there as my entries with 1 and causing overplotting at the bottom of the plot.) and reduced the y-axis scale to 40 so that plots are clear.
